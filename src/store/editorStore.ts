...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  loadDocument: (doc) => set((s) => ({
    ...
    // Any in-flight interaction session (mask edit/create, angle-adjust + its drag
    // fast-path, hover, pending strand) belongs to the OLD document — drop it all, like
    // the tab actions. In particular this clears angleEditTarget + dragging/dragMoving:
    // a Load triggered while the modeless Adjust-Angle dialog is open would otherwise
    // leave the fast-path engaged (shadows globally OFF, the old doc's static bake
    // reused) over the freshly loaded document until that stale dialog is closed.
    ...resetTransient(),
  })),
