...
// Default corpus - purpose-built TEST-CASE fixtures authored for this project's
// rendering fixes (generated by tools/make_fidelity_fixtures.mjs). Each is a
// small, readable scene targeting ONE behavior; CI renders each with the Qt
// oracle and the JS renderer and diffs them. The pattern: when a renderer change
// lands, add the JSON that proves it here. (The wider OSS-save fixtures -
...
const DEFAULT_CORPUS = [
  ...
  // Mask shadow scenes (mask_receiver variants): each flips one input the mask
  // shadow pipeline reads.
